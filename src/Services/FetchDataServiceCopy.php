...
class FetchDataServiceCopy implements FetchDataContract
{
    ...
    */
    ...
    public function only(array|string|Closure $columns): static
    {
        ...
        $this->onlyColumns = self::uniqueColumns([...$this->onlyColumns, ...$columns,]);
        return $this;
    }
    ...
    public function withCount(array|string|Closure $relations): static
    {
        ...
        $this->withCountRelations = self::mergeRelations($this->withCountRelations, $relations);
    ...
    public function search(array|string|Closure $columns): static
    {
        ...
        $this->searchColumns = self::uniqueColumns([...$this->searchColumns, ...$columns,]);
    ...
    */
    ...
    ): Collection|LengthAwarePaginator
    {
        ...
        $relations = self::mergeRelations(
            $this->withRelations,
            self::normalizeRelations($with)
        );

        $counts = self::mergeRelations(
            $this->withCountRelations,
        ...
        $searchColumns = self::uniqueColumns([
            ...$this->searchColumns,
...
/************************/
    ...
    */
    ...
    /**
     * Resolve fluent configuration Closure.
     *
     * Closures used by fluent methods are configuration callbacks.
     *
     * Example:
     *
     * ->only(fn () => ['id', 'name'])
     ...
     * ->perPage(fn () => 20)
     *
     * The service instance is passed to the Closure as well, so this is
     * also supported:
     *
     * ->only(
     *     fn (FetchDataService $fetch) =>
     *         ['id', 'name']
     * )
     *
     * The Closure is resolved only once.
     */
    ...
    */
    ...
    private static function newModelQuery(
        string $modelClass
    ): Builder
    {
    ...
    */
    ...
    private static function applyEagerLoading(
        Builder $query,
        array   $relations
    ): void
    {
    ...
    */
    ...
    private static function applySelection(
        Builder $query,
        array   $only,
        array   $with = []
    ): void
     ...
     */
    private static function addRelationKeys(
        Model $model,
        array &$columns,
        array $relations
    ): void
    {
        foreach ($relations as $relationDefinition) {
            if (!is_string($relationDefinition)) {
                continue;
            }

            $relationName = explode(
                ':',
                $relationDefinition,
                2
            )[0];
    ...
    */
    ...
    ): Builder
    {
        ...
        $keyword = mb_substr(
            $keyword,
            0,
            self::MAX_SEARCH_KEYWORD_LENGTH
        );
        ...
        return $query->where(
            function (Builder $q) use (
                ...
                foreach ($searchColumns as $definition) {
                    ...
                    $condition = self::parseSearchCondition(
                        $definition,
                        $keyword
                    );
                    ...
                    if (str_contains($column, '.')) {
                        $relation = self::extractRelationName(
                            $column
                        );

                        $field = self::extractColumnName(
                            $column
                        );
    ...
    private static function parseSearchCondition(
    ...
    private static function extractRelationName(
        string $path
    ): string
    {
        $parts = explode('.', $path);
    ...
    private static function extractColumnName(
        string $path
    ): string
    {
        return (string)last(
    ...
    */
    ...
    {
        $defaultOrderBy = config(
            'handler.default_order_by',
            'created_at'
        );

        $defaultSort = strtolower(
            (string)config(
                'handler.default_sort_direction',
                'desc'
            )
        );
    ...
    */
    ...
    private function resolvePerPage(
        null|int|false $perPage
    ): int|false
    {
        /*
         * Fluent value has priority.
         */
        if ($this->perPageValue !== null) {
         ...
         * Request.
         */
        $requestPerPage = $this->request->integer(
            'per_page'
        );
    ...
    private function resolveLimitPagination(
        null|false|int $limitPagination
    ): int|false
    {
        /*
         * Fluent value has priority.
         ...
         * get() argument.
         */
        if ($limitPagination !== null) {
            return $limitPagination;
        }

        return (int)config(
            'handler.limit-pagination',
            250
        );
    }

    private static function applyPagination(
        Builder   $query,
        int|false $perPage,
        int|false $limitPagination
    ...
    */
    ...
    private static function registerCacheKey(
        string $key
    ): void
    {
        $registry = Cache::get(
            self::COLUMNS_REGISTRY_KEY,
            []
        );
    ...
    private static function columnsCacheKey(
        string  $table,
        ?string $connection
    ): string
    {
        return sprintf(
            'fetch-data-columns:%s:%s',
            $connection ?: 'default',
            $table
        );
    }
    ...
    */
     ...
     */
    private static function mergeRelations(array $first, array $second): array
    {
        $result = [];
     ...
     */
    private static function mergeColumns(
        array $fluentColumns,
        array $getColumns
    ): array
    {
        if (
            in_array('*', $fluentColumns, true)
            || in_array('*', $getColumns, true)
        ) {
            return ['*'];
        }
